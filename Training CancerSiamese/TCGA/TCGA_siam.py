...
if train == 'Transfer_learning':
    with open(file_path + 'wght_pretrained19.pkl', "rb") as f:
...
def get_siamese_model(input_shape):
    ...
    model.add(Conv1D(filters=256, kernel_size=50, strides=50, activation='relu', weights=wghts_pretrained[0], padding='same',
                     input_shape=input_shape))
    model.add(Conv1D(filters=128, kernel_size=10, strides=1, activation='relu', weights=wghts_pretrained[1], padding='same'))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(filters=128, kernel_size=5, strides=1, activation='sigmoid', weights=wghts_pretrained[2], padding='same'))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Flatten())
    ...
    siamese_net = Model(inputs=[left_input, right_input], outputs=last_layer(encoded_l, encoded_r, lyr_name='L2')) ## prediction and cosine_similarity
    return siamese_net
